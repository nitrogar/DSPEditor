...
#define DECLARE_KEY_TESTER(Key)                                                \
  ...
  struct KeyTester_##Key {                                                     \
    template <typename T>                                                      \
    static int Get(typename std::enable_if<has_nested_##Key<ImGuiKey>::value,  \
                                           T>::type *) {                       \
...
void ed::Node::DrawBorder(ImDrawList *drawList, ImU32 color, float thickness) {
  if (thickness > 0.0f) {
    drawList->AddRect(m_Bounds.Min, m_Bounds.Max, color, m_Rounding,
...
void ed::EditorContext::Begin(const char *id, const ImVec2 &size) {
  ...
  if (!m_IsInitialized) {
    // Cycle canvas so it has a change to setup its size before settings are
    // loaded
...
bool ed::NavigateAction::HandleZoom(const Control &control) {
  ...
  auto newZoom =
      MatchZoom(steps, m_ZoomLevels[steps < 0 ? 0 : m_ZoomLevelCount - 1]);
