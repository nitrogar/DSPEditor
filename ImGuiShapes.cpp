...
void ImGui::DrawCircle(float radius, ImU32 color, float segments,
                       float thickness) {
  ImDrawList *list = ImGui::GetWindowDrawList();
  ImVec2 pos = ImGui::GetCursorPos() + ImGui::GetWindowPos();
  list->AddCircle(ImVec2(pos.x + radius, pos.y + radius), radius, color,
                  (int)segments, thickness);
...
void ImGui::DrawFilledCircle(float radius, ImU32 color, float segments) {
  ImDrawList *list = ImGui::GetWindowDrawList();
  ImVec2 pos = ImGui::GetCursorPos() + ImGui::GetWindowPos();
  list->AddCircleFilled(ImVec2(pos.x + radius, pos.y + radius), radius, color,
                        (int)segments);
  // ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2(2*radius , 2*radius));
}

void ImGui::DrawRect(ImVec2 size, ImU32 color, float rounding,
                     float thickness) {
  ImDrawList *list = ImGui::GetWindowDrawList();
  ImVec2 pos = ImGui::GetCursorPos() + ImGui::GetWindowPos();
